...
three_digits_set = {x*base**p+base*y+z for p in digits for x in digits for y in digits for z in digits if p < 3}
...
L2 = ['A', 1, 2, 3] # <-- same len(L2) == len(list(set(L2))) but L2 != list(set(L2))

## Task 18
odd_num_list_range = {x for x in list(range(100))[1::2]} # [x for x in range(100) if x%2 == 1]  #list(range(100))[1::2]
...
## Task 20
list_sum_zip = [x1+x2 for (x1, x2) in zip([10, 25, 40], [1, 15, 20])]
...
base = 10
digits = set(range(10))
representation_dict = {x*base**2+base*y+z:[x, y, z] for x in digits for y in digits for z in digits}

## Task 26
d = {0:1000.0, 1:1200.50, 2:990}
names = ['Larry', 'Curly', 'Moe']
listdict2dict = {names[k]:v for (k, v) in d.items()} # {k:d[v] for (k, v) in list(zip(L, range(len(L)))) if len(k) > 0}  # {L[k]:v for (k, v) in d.items()}
